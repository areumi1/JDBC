...
public class AccUI implements ActionListener
{
	...
	public void setup(){
		...
		confirmDia.setLayout( new BorderLayout(20,20));
		confirmDia.add("Center", p_diaCenter);
		confirmDia.add("South", p_diaSouth );
	...
	public void actionPerformed( ActionEvent ev ){
		...
		if ( o == b_moveMoney )
		{
			...
			// 1. 화면에서 입력값 얻어오기
				String  sendAccNum = tf_sendAccNum.getText();
				String  recvAccNum = tf_recvAccNum.getText();
				int		moveMoney  = Integer.parseInt(tf_moveMoney.getText());
			...
			// 3. BL객체에서 계좌 이체하는 함수 호출 ( ex. moveAccount() )
				...
				int result = logic.moveAccount(sendAccNum, recvAccNum, moveMoney);
				if ( result == 0) JOptionPane.showMessageDialog(null, "이체성공");
				else if ( result == -1 ) JOptionPane.showMessageDialog(null, "이체실패");
